...
const casteMappings: Record<string, string[]> = {
  "Hindu": ["Brahmin","Kshatriya","Vaishya","Shudra","Agarwal","Arora","Aryavyshya","Balija","Bania","Chettiar","CNB","Devanga","Ezhava","Goud","Gounder","Iyer","Iyengar","Jat","Kamma","Kapu","Kayastha","Khandayat","Khatri","Kongu Vellalar","Kurmi","Lingayat","Maratha","Meena","Mudaliar","Nadar","Naidu","Nair","Padmashali","Patel","Pillai","Rajput","Reddy","Saini","Sengunthar","Thevar","Vadera","Vanniyar","Velama","Vishwakarma","Vokkaliga","Yadav","SC / ST","OBC","Other","No Caste Preference"],
  "Muslim": ["Ansari","Hanafi","Jat Muslim","Khoja","Lebbai","Malik","Mappila","Memon","Mughal","Pathan","Qureshi","Rajput Muslim","Rowther","Sayyid","Shafi","Sheikh","Siddiqui","Sunni","Shia","CNB","Other","No Caste Preference"],
  "Christian": ["Anglo-Indian","Born Again","Catholic","CNI","CSI","Jacobite","Knanaya","Latin Catholic","Marthoma","Nadar Christian","Orthodox","Pentecostal","Protestant","Roman Catholic","Syrian Catholic","Syrian Orthodox","CNB","Other","No Caste Preference"],
  "Sikh": ["Arora","Bhatia","Ghumar","Jat Sikh","Kamboj","Khatri","Labana","Mazhabi","Ramgarhia","Ravidasia","Saini","CNB","Other","No Caste Preference"],
  "Jain": ["Agarwal","Digamber","Jaiswal","KVO","Oswal","Porwal","Shwetamber","Visa Oswal","CNB","Other","No Caste Preference"],
  "Buddhist": ["Mahar","Neo-Buddhist","Theravada","Mahayana","Vajrayana","CNB","Other","No Caste Preference"],
                    ...
                    {pendingProfiles.map((p, i) => (
                      ...
                      >
                        <div className="flex items-center gap-3 px-4 py-3">
                          ...
                          {(p as any).profile_id && (
                            <span className="px-2 py-1 rounded-lg text-xs font-bold flex-shrink-0 hidden sm:inline" style={{ background: "hsl(210, 80%, 93%)", color: "hsl(210, 80%, 35%)" }}>{(p as any).profile_id}</span>
                          )}
                          <StatusBadge status={p.profile_status} />
                        </div>
                        <div className="flex items-center gap-2 px-4 pb-3 flex-wrap">
                          {(p as any).profile_id && (
                            <span className="px-2 py-1 rounded-lg text-xs font-bold sm:hidden" style={{ background: "hsl(210, 80%, 93%)", color: "hsl(210, 80%, 35%)" }}>{(p as any).profile_id}</span>
                          )}
                  ...
                  <div className="space-y-3">
                    {filteredAllProfiles.map((p, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 px-4 pb-3 flex-wrap">
                          {(p as any).profile_id && (
                            <span className="px-2 py-1 rounded-lg text-xs font-bold" style={{ background: "hsl(210, 80%, 93%)", color: "hsl(210, 80%, 35%)" }}>{(p as any).profile_id}</span>
                          )}
                          <span className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-semibold" style={{ background: "hsl(210, 80%, 96%)", color: "hsl(210, 80%, 45%)" }}>
                            <Eye size={12} /> View
                          </span>
