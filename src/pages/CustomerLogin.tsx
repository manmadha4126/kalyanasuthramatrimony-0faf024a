...
export default function CustomerLogin() {
  ...
  return (
    <div className="min-h-screen flex overflow-hidden" style={{ background: "hsl(30, 33%, 97%)" }}>
      {/* Left Panel */}
      <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="hidden lg:flex flex-col justify-center w-1/2 relative overflow-hidden" style={{ background: "linear-gradient(145deg, hsl(348, 56%, 20%) 0%, hsl(340, 50%, 28%) 50%, hsl(30, 40%, 25%) 100%)" }}>
        ...
        <div className="relative z-10 px-12 py-16">
          ...
          <h1 className="text-3xl font-serif font-bold text-white mb-3 leading-snug">The Wedding<br />Chapter Starts Here…</h1>
          <p className="text-white/60 text-sm leading-relaxed mb-10 max-w-xs">Thousands of hearts found their perfect match. Your love story is waiting to begin.</p>
          <div className="relative h-52">
            {decorativeCards.map((card, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.15 }} className="absolute rounded-2xl overflow-hidden shadow-2xl" style={{ width: "140px", height: "170px", left: `${i * 90}px`, top: `${i % 2 === 1 ? 20 : 0}px`, zIndex: i, transform: `rotate(${i === 0 ? -5 : i === 1 ? 0 : 5}deg)`, border: "3px solid hsl(42, 42%, 57% / 0.6)" }}>
              ...
              <>
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={identifier} onChange={e => setIdentifier(e.target.value)} placeholder="your@email.com or 9876543210" required className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all" />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      <input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter password" required className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 pr-10 transition-all" />
                  ...
                  <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-semibold text-sm text-white transition-all disabled:opacity-60 hover:scale-[1.01]" style={{ background: "linear-gradient(135deg, hsl(var(--burgundy)), hsl(var(--burgundy-deep)))" }}>
