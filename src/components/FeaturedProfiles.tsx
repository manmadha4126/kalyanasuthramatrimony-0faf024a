      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="mb-14 mt-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6 max-w-6xl mx-auto">
            {grooms.length > 0
              ? grooms.slice(0, 18).map((p, i) => <ProfileCard key={p.id} profile={p} index={i} />)
              : Array.from({ length: 18 }).map((_, i) => <PlaceholderCard key={`gp${i}`} index={i} />)
            }
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6 max-w-6xl mx-auto">
            {brides.length > 0
              ? brides.slice(0, 18).map((p, i) => <ProfileCard key={p.id} profile={p} index={i} />)
              : Array.from({ length: 18 }).map((_, i) => <PlaceholderCard key={`bp${i}`} index={i} />)
            }
          </div>
        </div>
